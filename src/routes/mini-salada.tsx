...
export const Route = createFileRoute("/mini-salada")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Mini salada QUASE! no Torres de Olinda." },
    ],
  }),
      ...
      <div className="min-h-screen bg-background pb-12">
        ...
        <main className="mx-auto max-w-3xl px-5 pt-10">
          <section className="rounded-4xl border border-border bg-card p-7 text-center shadow-soft">
            ...
            <p className="mx-auto mt-2 max-w-sm text-sm text-muted-foreground">Hoje a cozinha está com mini saladas prontas para pedir.</p>
            <div className="mt-7 grid gap-3 sm:grid-cols-2">
              <Button variant="outline" size="lg" className="rounded-full" onClick={() => { setJustAdded(false); setQuantity(1); }}>Pedir outra</Button>
    ...
    <div className="min-h-screen bg-background pb-36">
      ...
      <main className="mx-auto max-w-3xl px-5 pt-8">
        <section className="rounded-4xl border border-border bg-card p-6 shadow-soft">
          ...
          <p className="mt-2 text-sm leading-relaxed text-muted-foreground">Alface, tomate, cebola roxa e pepino. Receita fixa para hoje.</p>
          <div className="mt-6 rounded-3xl bg-cream p-4"><p className="text-xs text-muted-foreground">Hoje vai com</p><p className="mt-2 text-sm font-medium">Alface · Tomate · Cebola roxa · Pepino</p></div>
          <div className="mt-4 flex items-center justify-between rounded-3xl bg-cream p-4">
            <div><p className="text-xs text-muted-foreground">Valor unitário</p><p className="font-display mt-1 text-2xl font-semibold">{formatBRL(PRICE)}</p></div>
            <div className="flex items-center gap-2">
              <button type="button" aria-label="Diminuir quantidade" disabled={quantity === 1} onClick={() => setQuantity((value) => Math.max(1, value - 1))} className="flex h-11 w-11 items-center justify-center rounded-full border border-border bg-background disabled:opacity-35"><Minus className="h-4 w-4" /></button>
              <span className="w-8 text-center text-xl font-semibold tabular-nums">{quantity}</span>
              <button type="button" aria-label="Aumentar quantidade" onClick={() => setQuantity((value) => value + 1)} className="flex h-11 w-11 items-center justify-center rounded-full bg-foreground text-background"><Plus className="h-4 w-4" /></button>
            </div>
          </div>
        </section>
      </main>
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-background/95 px-5 pt-3 pb-5 backdrop-blur-md">
        <div className="mx-auto flex max-w-3xl gap-2">
          <Button size="lg" className="flex-1 rounded-full" onClick={addToCart}>Adicionar · {formatBRL(PRICE * quantity)}</Button>
