...
export const Route = createFileRoute("/acai")({
  head: () => ({
    meta: [
...
function AcaiPage() {
  ...
  const categories = useMemo(() => {
    return categoriesForKind("acai").map((category) => {
      const baseCategory = {
        ...category,
        ingredients: category.ingredients.filter((ingredient) => isIngredientAvailable(ingredient.id)),
      };
  ...
  return (
    <div className="min-h-screen bg-background pb-56">
      ...
      <main className="mx-auto max-w-3xl px-5 pt-6">
        <p className="mb-4 text-center text-[11px] leading-relaxed text-muted-foreground">Nosso açaí é sorvete de açaí; não trabalhamos com açaí puro.</p>
        {fathersDay && <div className="mb-5 rounded-3xl bg-lavender/30 p-4 text-sm"><strong>Hoje tem açaí quase saudável!</strong> O especial de 750 ml vem com 3 frutas, 3 caldas e 10 complementos por R$ 25.</div>}
        <div className="flex items-center gap-1.5">{categories.map((category, index) => <div key={category.id} className={cn("h-0.5 flex-1 rounded-full", index <= step ? "bg-foreground" : "bg-border")} />)}</div>
